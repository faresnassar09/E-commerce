...
<li class="flex items-center gap-2">
  <img src="{{ asset('images/'.$item->product->images->first()->path) ?? 'default.jpg' }}" class="w-16 h-16 object-cover rounded border">
  
  <div>
    <p>{{ $item->product->name }}</p>
    <p class="text-xs text-gray-500">{{ __('messages.price') }}: {{ $item->price }} | {{ __('messages.much') }}: {{ $item->quantity }}</p>
...
function openSellerContact(phone) {
  document.getElementById('sellerPhoneText').textContent = phone ?? 'غير متوفر';
