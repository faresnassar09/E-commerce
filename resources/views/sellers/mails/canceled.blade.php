...
<head>
    <meta charset="UTF-8">
    <title>{{__('messages.order_cancelled_title')}}</title>
</head>
<body style="font-family: sans-serif; background-color: #f9fafb; padding: 20px; color: #333;">
    ...
    <div style="max-width: 600px; margin: auto; background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
        <h2 style="color: #dc2626; font-size: 24px;">❌ {{__('messages.order_cancelled_heading')}}</h2>

        <p style="font-size: 16px;">{{__('messages.order_cancelled_message', ['order_number' => $orderDetails['order_number']])}}</p>
        ...
        <table style="width: 100%; margin-top: 10px;">
            ...
            <tr>
                <td><strong>{{__('messages.cancellation_date')}}:</strong></td>
                <td>{{ \Carbon\Carbon::now()->format('Y-m-d H:i') }}</td>
            </tr>
        </table>

        <h3 style="margin-top: 30px; font-size: 18px;">🛒 {{__('messages.cancelled_products_heading')}}</h3>
        ...
        <p style="margin-top: 30px;">{{__('messages.cancellation_note')}}</p>
    </div>
