      ...
      @foreach ($products as $product)
        <div class="bg-white p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow relative">
          ...
          <div class="mt-2 text-right">
            @if($product->discount)
              <p class="text-sm text-red-500 line-through">{{ number_format($product->price, 2) }} EGP</p>
              <p class="text-green-500 font-bold">{{ number_format($product->price - $product->discount, 2) }} EGP</p>
            @else
              <p class="text-green-600 font-semibold">{{ number_format($product->price, 2) }} EGP</p>
            @endif
          </div>
...
<livewire:add-to-cart :id="$product->id"/>
  ...
  <section class="container mx-auto mt-20 px-4">
    <h2 class="text-xl sm:text-3xl font-bold mb-6 text-gray-800">{{ 'المتاجر' }}</h2>
      ...
      @foreach ($stores as $store)
        <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow p-5 flex flex-col items-center text-center">
          <div class="w-24 h-24 rounded-full overflow-hidden mb-4 border-4 border-orange-500">
            <img src="{{ Storage::url($store->images->first()?->path ?? 'default.jpg') }}" alt="Store Logo" class="w-full h-full object-cover">
          </div>

          <h3 class="text-lg font-bold text-gray-800 mb-1">{{ $store->name }}</h3>

          <p class="text-sm text-gray-500 mb-2">
            🏙️ {{ $store->city_name ?? '' }}<br>
